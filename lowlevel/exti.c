...
void exti9_5_isr(){
    if(exti_get_flag_status(ARM_LIMITSWITCH_EXTI)){
        //TODO: things to do when arm limit switch is pushed
    ...
    if(exti_get_flag_status(FLAG_LIMITSWITCH_EXTI)){
        //TODO: things to do when flag limit switch is pushed
...
void _arm_limit_switch_init(){
    _gpio_setup_pin(ARM_LIMITSWITCH_RCC,ARM_LIMITSWITCH_PORT,
    ARM_LIMITSWITCH_PIN,GPIO_MODE_INPUT, GPIO_PUPD_PULLUP,GPIO_OTYPE_PP);
    _limit_switch_init(ARM_LIMITSWITCH_EXTI,ARM_LIMITSWITCH_PORT,
    ARM_NVIC_INTERRUPT_NUMBER, EXTI_TRIGGER_FALLING);

    // uncomment to enable priority management
    // nvic_set_priority(ARM_NVIC_INTERRUPT_NUMBER, ARM_PRIORITY);
}

void _flag_limit_switch_init(){
    _gpio_setup_pin(FLAG_LIMITSWITCH_RCC,FLAG_LIMITSWITCH_PORT,
    FLAG_LIMITSWITCH_PIN,GPIO_MODE_INPUT, GPIO_PUPD_PULLUP,
    GPIO_OTYPE_PP);
    _limit_switch_init(FLAG_LIMITSWITCH_EXTI,FLAG_LIMITSWITCH_PORT,
    FLAG_NVIC_INTERRUPT_NUMBER, EXTI_TRIGGER_FALLING);
